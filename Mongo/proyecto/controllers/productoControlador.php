...
class productoControlador {
    ...
    public function ver_imagenes() {
        ...
        if (isset($_GET['id'])) {
            ...
            // Obtener las imágenes de un producto (ahora devuelve un array)
            $imagenes = $imagen->obtenerImagenesProducto($producto->getId());
    
            // Ruta base donde se almacenan las imágenes
            $rutaBaseImagenes = "uploads/images/";
    
            // Modificar las imágenes para que contengan la ruta completa
            foreach ($imagenes as $key => $img) {
                // Concatenar la ruta completa de la imagen usando la columna 'imagen_url'
                $imagenes[$key]['ruta_completa'] = $rutaBaseImagenes . $img['imagen_url']; // Cambié 'nombre_imagen' por 'imagen_url'
            }
    
            // Incluir la vista que mostrará las imágenes
    ...
    public function editar_promocion() {
        ...
        if (isset($_GET['id']) && is_numeric($_GET['id'])) {
            var_dump($_GET['id']);  // Agrega esta línea para depurar el valor de 'id'
